...
const standardMessages = {
    ...
    tidak: "Mohon maaf, Anda dinyatakan TIDAK LULUS seleksi Asisten Fisika Laboratorium 1. Tetap semangat dan terima kasih atas partisipasinya."
};

// Fungsi untuk menampilkan hasil modal
...
// Check if data exists in Firebase, if not, initialize with default data
function initializeData() {
    console.log("Memeriksa data di Firebase...");
    database.ref('candidates').once('value', snapshot => {
        if (!snapshot.exists()) {
            ...
            defaultCandidates.forEach((candidate, index) => {
                // Tambahkan pesan standar berdasarkan status
                const candidateWithDetails = {
                    ...candidate
                };
                
                database.ref('candidates/' + index).set(candidateWithDetails)
                    .then(() => console.log("Data default berhasil ditambahkan:", candidate.nrp))
                    .catch(error => console.error("Error saat menambahkan data default:", error));
            });
        } else {
